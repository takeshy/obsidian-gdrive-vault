...
1. ローカルでファイルを削除
2. Push → メタデータからファイルが削除される
3. **ファイルはGoogle Drive上に「未追跡」として残る**

**復元方法:** 設定 → 未追跡ファイルを検出 → ファイル選択 → 選択を復元

### シナリオ2: Full Pushで誤って上書き

1. 異なるローカルバージョンでFull Push
2. **古いリモートファイルがタイムスタンプ付きでリネーム（未追跡）**

**復元方法:** 設定 → 未追跡ファイルを検出 → 古いバージョン選択 → 選択を復元

### シナリオ3: Full Pullで誤って上書き

1. 異なるリモートバージョンでFull Pull
2. **古いローカルファイルが`sync_conflicts/`に保存**

**復元方法:** `sync_conflicts/`フォルダを参照してファイルをコピー
